extends Chum
class_name Chum14
var chum_id := 14
var chum_str := "chum14"
var chum_name := "Clop"

var desc := "One eyed one horned flying purple people eater. But doesnt fly, isnt purple and doesnt often eat people."

var min_attack_speed := 2.55

var base_attack_speed := 3.0
var base_attack_damage := 50
var base_move_speed := 0.75
var base_health := 250
#Control of qualities
var has_attack_speed := true
var has_attack_damage := true
var has_move_speed := true
var has_health := true

var attack_distance := 1.25
var knockback_strength := 17.5
var knockback_weight := 20.0

#Chance that it maintains agro on chasing chum being hit
var maintains_agro := 0.0
var changes_agro_on_damaged := true
var draws_agro_on_attack := true
var always_targets_player := false

var bracelet_count := 5 if not Global.dev_mode else 1
var bracelet_cost := 15 if not Global.dev_mode else 1
